...
#     prompt = f""" Convert the following natural language query into an SQL querie based on user questions about data stored in an RDF graph, using the given ontology. Ensure the output does not contain any special characters like \\n , and without any explanations.
...
# Given a user's question about this data, write a valid SQL query that accurately extracts or calculates the requested information from the RDF graph and adheres to SQL best practices, optimizing for readability and performance where applicable.


# Here are some tips for writing SQL queries:
# * Ensure queries are readable and well-structured.
# * Use descriptive variable names.
# * Filter results where necessary.

# Also note that:
# * Valid values for ds6w:type include 'vpmReference'

# Example Question:
# --------
# What are Physical Products created between May 10, 2024, and May 13, 2024, and modified today?
# --------
# Expected Output:
# --------
# Provide an SQL WHERE clause that filters records based on the creation and modification dates and the type of the product. Do not include any explanation, and format the output as an SQL statement.
# --------
# Example SQL Query Response WHERE Clause:
# --------
# {{
#     "sql": "ds6w:created >= '2024-05-10' AND ds6w:created <= '2024-05-13' AND ds6w:modified = CURRENT_DATE AND ds6w:type = 'VPMReference'"
# }}
# --------
...
# {query}
# --------
# Reminder: Generate an SQL query to answer the question:
# * respond as a valid JSON Document
# * [Best] If the question can be answered with the available ontology: {{"sql": <sql here>}}
# * If the question cannot be answered with the available ontology: {{"error": <explanation here>}}
# """
#     # try:
...
# def get_reflection_for_sql(full_prompt, llm_response):
#     reflection_prompt = f'''
...
#     1. SQL is valid SQL syntax, given the provided RDF metadata
#     2. The query specifically references the correct RDF datamodel
...
def query_groq():
    ...
    logging.debug("User Query: " + user_query)
    # sql_query,sql_prompt = text_to_sql(user_query)
    # final_sql_query=get_reflection_for_sql(sql_prompt,sql_query)
